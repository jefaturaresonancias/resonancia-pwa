...
const ConfigView = (() => {
  ...
  // Rediseño 28/8/2026 (referencia provista por el usuario) — bloques
  // autoexplicados (nombre+rango adentro), color = categoría (no
  // identidad), y los cupos de turnos-por-franja alineados al mismo eje
  // horario que la grilla, en vez de vivir en una tabla sin relación
  // visual. Coloreada con la MISMA lógica de prioridades que la agenda
  // real (api_agenda_previewSemanal, rpc/agenda.js) para que esta vista
  // nunca muestre algo distinto de lo que realmente bloquea. Todo sale de
  // datos reales (_panoramaDias/_limitesCache/_limitesFranjaCache) — nada
  // hardcodeado, a diferencia del prototipo de referencia.
  const PANORAMA_ORDEN_DIAS = [1, 2, 3, 4, 5, 6, 0];
  const PANORAMA_DIAS_LABEL = { 0: 'Dom', 1: 'Lun', 2: 'Mar', 3: 'Mié', 4: 'Jue', 5: 'Vie', 6: 'Sáb' };
  const PANORAMA_ROW_H = 30; // px por hora — altura total = 24 * ROW_H
  ...
  function _gruposCupos() {
    const activos = (_limitesFranjaCache || []).filter(r => r.activa !== false);
    ...
    return [...porFirma.values()].map(g => {
      ...
      g.reglas.forEach(r => {
        ...
        bandas.push({ from: desde, to: hasta, limite: r.limite, range: `${r.horaDesde}–${r.horaHasta}` });
        cursor = Math.max(cursor, hasta);
      });
      if (cursor < 1440) bandas.push({ from: cursor, to: 1440, limite: null });
      const label = g.dias.length ? g.dias.map(d => PANORAMA_DIAS_LABEL[d]).join('/') : 'Todos los días';
      return { label, bandas };
    });
  }
  ...
  function _seccionPanoramaSemanal() {
    ...
    const totalH = PANORAMA_ROW_H * 24;

    const leyenda = [
      { label: 'Cerrado / bloqueo', sw: PANO_STYLES.bloqueo.bg },
    ...
    const headerDias = PANORAMA_ORDEN_DIAS.map(dia =>
      `<div style="flex:1;min-width:0;text-align:center"><div style="font-size:12.5px;font-weight:650">${PANORAMA_DIAS_LABEL[dia]}</div></div>`
    ).join("");

    const gutter = [];
    for (let h = 0; h <= 24; h += 2) {
      gutter.push(`<span style="position:absolute;right:8px;top:${h*PANORAMA_ROW_H}px;transform:translateY(-50%);font-size:10.5px;color:#9a938a;font-family:ui-monospace,SFMono-Regular,Menlo,monospace">${h === 24 ? '24' : String(h).padStart(2,'0')}</span>`);
    }

    const columnas = PANORAMA_ORDEN_DIAS.map(dia => {
      const bandas = _mergeBandas(porDia[dia]);
      const bloques = bandas.map(b => {
        const st = PANO_STYLES[b.categoria] || PANO_STYLES.estudio;
        const h = (b.to - b.from) / 60 * PANORAMA_ROW_H;
        const alto = h >= 30; // ~1h o más: entra el rango de horario adentro
        return `<div title="${b.label} · ${_horaMin(b.from)}–${_horaMin(b.to)}"
          style="position:absolute;left:3px;right:3px;top:${b.from/60*PANORAMA_ROW_H}px;height:${h-2}px;border-radius:3px;overflow:hidden;padding:3px 5px;box-sizing:border-box;background:${st.bg};border:${st.border};color:${st.fg};display:flex;flex-direction:column;justify-content:center;gap:1px;cursor:default">
      ...
      return `<div style="flex:1;min-width:0;position:relative;height:${totalH}px;border:1px solid #eae5df;border-radius:4px;background-color:#fcfbfa;background-image:repeating-linear-gradient(to bottom,#eae5df 0 1px,rgba(0,0,0,0) 1px ${PANORAMA_ROW_H*2}px)">${bloques}</div>`;
    }).join("");

    const gruposCupos = _gruposCupos();
    const cabecerasCupos = gruposCupos.map(g => `<div style="width:150px;flex-shrink:0;padding-left:14px;font-size:10.5px;font-weight:650;letter-spacing:.06em;text-transform:uppercase;color:#8a8279">Cupo ${g.label}</div>`).join("");
    const columnasCupos = gruposCupos.map(g => {
      const bandas = g.bandas.map(c => {
        ...
        return `<div style="position:absolute;left:14px;right:0;top:${c.from/60*PANORAMA_ROW_H}px;height:${(c.to-c.from)/60*PANORAMA_ROW_H}px;box-sizing:border-box;border-left:3px solid ${bar};padding:4px 0 4px 10px;display:flex;flex-direction:column;justify-content:center">
    ...
    return `<div style="margin-bottom:1.5rem">
      <div style="display:flex;justify-content:space-between;align-items:flex-end;gap:24px;margin-bottom:14px;flex-wrap:wrap">
        ...
        <div style="display:flex;gap:8px;flex-wrap:wrap;justify-content:flex-end">${leyenda}</div>
      </div>
      ...
      <div style="background:var(--surface);border:0.5px solid var(--border);border-radius:6px;padding:18px 20px 22px">
        <div style="display:flex;align-items:flex-end;gap:0;padding-left:46px;padding-right:8px;margin-bottom:8px">
          <div style="flex:1;display:flex;gap:6px">${headerDias}</div>
          ${cabecerasCupos}
        </div>
        <div style="display:flex;align-items:stretch;overflow-x:auto">
          <div style="width:46px;flex-shrink:0;position:relative;height:${totalH}px">${gutter.join("")}</div>
          <div style="flex:1;display:flex;gap:6px;min-width:600px">${columnas}</div>
          ${columnasCupos}
        </div>
      </div>
  ...
  function _diasAbrev(dias) {
    return (dias && dias.length) ? dias.map(d => PANORAMA_DIAS_LABEL[d]).join('/') : 'todos los días';
  }
  ...
  function _tablaLimitesResumen() {
    const filasSobreturno = (_limitesCache || []).filter(r => r.activa !== false).map(r => `
      <div style="display:flex;align-items:baseline;gap:12px;padding:7px 0;border-top:1px solid #f0ece6">
        ...
        <div style="font-size:12px;flex:1;min-width:0">${(PANO_QUE_SOBRETURNO[r.ambito]||(()=>r.ambito))(r)}</div>
